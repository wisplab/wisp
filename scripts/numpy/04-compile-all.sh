...
COMMON_FLAGS=(
  ...
  -I"$GEN"
  # numpy.random subtree (needed for the random .c files; harmless for others)
  -I"$NUMPY_DIR/numpy/random"
  -I"$NUMPY_DIR/numpy/random/src"
  -I"$NUMPY_DIR/numpy/random/include"
)
...
# numpy.fft._pocketfft_internal — one .c file, no extra deps.
# Compiles against the same include set as multiarray.
FFT_SRC=(
  ../fft/_pocketfft.c
)

# numpy.linalg._umath_linalg — C++ wrapper plus the 9 f2c-translated
# reference BLAS+LAPACK .c files in numpy/linalg/lapack_lite/. No
# Fortran toolchain needed; numpy ships these as the fallback when
# no system BLAS is found, exactly our scenario on wasm.
LINALG_SRC=(
  ../linalg/lapack_lite/f2c.c
  ../linalg/lapack_lite/f2c_config.c
  ../linalg/lapack_lite/f2c_blas.c
  ../linalg/lapack_lite/f2c_lapack.c
  ../linalg/lapack_lite/f2c_s_lapack.c
  ../linalg/lapack_lite/f2c_d_lapack.c
  ../linalg/lapack_lite/f2c_c_lapack.c
  ../linalg/lapack_lite/f2c_z_lapack.c
  ../linalg/lapack_lite/python_xerbla.c
)
LINALG_CPP=(
  ../linalg/umath_linalg.cpp
)

# numpy.random — 9 cythonized .c files (generated by 01b-cython.sh)
# plus the shared C support libs (distributions, PRNG algorithms).
# Each Cython extension is registered in the inittab dotted under
# `numpy.random.<modname>` (see wisp_entry.c).
RANDOM_CYTHONIZED=(
  ../random/_common.c
  ../random/bit_generator.c
  ../random/_bounded_integers.c
  ../random/_generator.c
  ../random/_mt19937.c
  ../random/_pcg64.c
  ../random/_philox.c
...
RANDOM_SUPPORT_SRC=(
  ../random/src/distributions/logfactorial.c
  ../random/src/distributions/distributions.c
  ../random/src/distributions/random_mvhg_count.c
  ../random/src/distributions/random_mvhg_marginals.c
  ../random/src/distributions/random_hypergeometric.c
  ../random/src/legacy/legacy-distributions.c
  ../random/src/mt19937/mt19937.c
  ../random/src/mt19937/mt19937-jump.c
  ../random/src/philox/philox.c
  ../random/src/pcg64/pcg64.c
  ../random/src/sfc64/sfc64.c
)
...
compile_c() {
  local rel="$1"
  shift  # remaining args are extra clang flags for this file only
  ...
  # Output object name: dot-flatten directory separators, then strip
  # leading dots so `ls` and `ar objs/*.o` don't silently skip the
  # file (dotfiles are excluded by both default globs).
...
echo "==> Compiling fft .c (${#FFT_SRC[@]} files)"
for f in "${FFT_SRC[@]}"; do compile_c "$f"; done

echo "==> Compiling linalg lapack_lite .c (${#LINALG_SRC[@]} files)"
for f in "${LINALG_SRC[@]}"; do compile_c "$f"; done

echo "==> Compiling linalg .cpp (${#LINALG_CPP[@]} files)"
for f in "${LINALG_CPP[@]}"; do compile_cpp "$f"; done

echo "==> Compiling random support .c (${#RANDOM_SUPPORT_SRC[@]} files)"
for f in "${RANDOM_SUPPORT_SRC[@]}"; do compile_c "$f"; done

# Cythonized random files use the OLD numpy C API names (NPY_ALIGNED,
# NPY_FORCECAST, etc.). Those are hidden behind NPY_NO_DEPRECATED_API
# unless that macro is set to 0. numpy's own setup.py for random does
# `defs.append(('NPY_NO_DEPRECATED_API', 0))` for this same reason.
echo "==> Compiling random cythonized .c (${#RANDOM_CYTHONIZED[@]} files)"
for f in "${RANDOM_CYTHONIZED[@]}"; do
  compile_c "$f" -UNPY_NO_DEPRECATED_API -DNPY_NO_DEPRECATED_API=0
done
